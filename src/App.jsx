...
export default function App() {
  ...
  if (screen === "dashboard" && session) {
    ...
    return (
      <DashboardLayout session={session} userRole={userRole} onLogout={handleLogout}
        toast={toast} onHome={() => setScreen("home")} initials={initials()}>
        {rol === "super_admin"  && <SuperAdmin session={session} />}
        {rol === "league_admin" && <LeagueAdmin session={session} />}
        {rol === "referee"      && <Referee session={session} />}
        {rol === "player"       && <PlayerProfile session={session} />}
      </DashboardLayout>
    );
  }

  // Unidad deportiva page
...
// DASHBOARD LAYOUT
// ─────────────────────────────────────────────────────────────────
function DashboardLayout({ children, session, userRole, onLogout, toast, onHome, initials }) {
  const [open, setOpen] = useState(true);
  const rol = userRole?.rol;
  const roleInfo = ROLES_INFO[rol] || { label:"Usuario", icon:"👤", color:"#666" };
  const menu = {
    super_admin:  [["👑","Panel Admin"],["🏟️","Canchas"],["🏆","Torneos"],["📊","Clasificación"],["👥","Usuarios"],["📋","Solicitudes"]],
    league_admin: [["🏟️","Mi Torneo"],["👕","Equipos"],["🏆","Torneos"],["📊","Clasificación"],["👥","Jugadores"]],
    referee:      [["🟡","Mis Partidos"],["📝","Ficha de Partido"]],
    player:       [["⚽","Mi Perfil"],["👕","Mi Equipo"],["🏆","Torneos"]],
  };
  return (
    <div style={{ display:"flex", flexDirection:"column", minHeight:"100vh", background:"var(--bg)" }}>
      ...
      <header style={s.topbar}>
        <div style={s.topLeft}>
          <button className="ham-btn" onClick={() => setOpen(!open)}>☰</button>
          <div style={s.brand} onClick={onHome} className="clickable">
            <div style={s.brandIcon}><BallIcon /></div>
            <span style={s.brandName}>IFútbol</span>
          </div>
        ...
        <div style={s.topRight}>
          <div style={{ ...s.pill, background: roleInfo.color+"18", color: roleInfo.color, border:`1px solid ${roleInfo.color}30` }}>{roleInfo.icon} {roleInfo.label}</div>
      ...
      <div style={{ display:"flex", flex:1 }}>
        <aside style={{ width:open?220:0, overflow:"hidden", transition:"width 0.25s ease", background:"white", borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", position:"sticky", top:60, height:"calc(100vh - 60px)" }}>
          <nav style={{ flex:1, padding:"14px 10px", display:"flex", flexDirection:"column", gap:3 }}>
            {(menu[rol]||[]).map(([icon,label]) => (
              <div key={label} className="nav-item">{icon} {label}</div>
            ))}
          </nav>
          <div style={s.sbFooter}>
            <Avatar initials={initials} size={36} />
            <div style={{ overflow:"hidden" }}>
              <div style={{ fontSize:12, fontWeight:700, color:"var(--text)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{session?.user?.email?.split("@")[0]}</div>
              <div style={{ fontSize:11, color:"var(--text-muted)" }}>{roleInfo.label}</div>
            </div>
          </div>
        </aside>
        <main style={{ flex:1, overflow:"auto", padding:"32px 36px" }}>
          <div style={{ maxWidth:960, margin:"0 auto" }} className="animate-in">{children}</div>
        </main>
      </div>
    </div>
  );
...
function HomePage({ canchas, onVerUnidad }) {
  const [pubImgs] = useState([]);
  const [pubIdx, setPubIdx] = useState(0);

  useEffect(() => {
    if (pubImgs.length <= 1) return;
    const t = setInterval(() => setPubIdx(i => (i + 1) % pubImgs.length), 12000);
    ...
    <div className="animate-in">
      ...
      {torneoActivo && <>
        ...
        {loading ? <div style={{ padding:60, textAlign:"center" }}><div className="spinner"/></div> : <>
          ...
          {/* TABLA */}
          {seccion==="tabla" && (
            clasificacion.length===0 ? <EmptyState icon="📊" txt="No hay partidos jugados aún" /> :
          ...
          {/* PARTIDOS */}
          {seccion==="partidos" && (
            partidos.length===0 ? <EmptyState icon="📅" txt="No hay partidos registrados aún" /> :
              ...
              {partidos.map(p => {
                ...
                return (
                  <div key={p.id} style={{ background:"white", borderRadius:"var(--radius-md)", padding:"16px 20px", boxShadow:"var(--shadow-sm)", border:"1px solid var(--border)" }}>
                    ...
                    {(f.goleadores||[]).length>0 && <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:10 }}>
                      {(f.goleadores||[]).map((g,i) => <span key={i} style={{ background:"var(--green-light)", color:"var(--green)", fontSize:11, padding:"3px 10px", borderRadius:6, fontWeight:600 }}>⚽ {g.nombre} ({g.goles})</span>)}
                    </div>}
                  </div>
                );
              })}
          ...
          {/* GOLEADORES */}
          {seccion==="goleadores" && (
            goleadores.length===0 ? <EmptyState icon="🥇" txt="No hay goles registrados aún" /> :
              ...
              {goleadores.map((g,i) => (
                <div key={i} style={{ background:"white", borderRadius:"var(--radius-md)", padding:"13px 20px", boxShadow:"var(--shadow-sm)", border:"1px solid var(--border)", display:"flex", alignItems:"center", gap:14 }}>
                  ...
                  <div style={{ width:70, height:6, background:"#f3f4f6", borderRadius:3, overflow:"hidden" }}>
                    <div style={{ height:"100%", background:"var(--green)", width:`${(g.goles/(goleadores[0]?.goles||1))*100}%`, borderRadius:3 }}/>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* EQUIPOS */}
          {seccion==="equipos" && (
            equipos.length===0 ? <EmptyState icon="👕" txt="No hay equipos registrados" /> :
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(160px,1fr))", gap:14 }}>
              {equipos.map(eq => {
                const st = clasificacion.find(c=>c.equipo.id===eq.id);
                return (
  ...
  return (
    <div style={{ background:"white", borderRadius:"var(--radius-md)", overflow:"hidden", boxShadow:"var(--shadow-md)" }}>
      ...
      <table className="ifutbol-table">
        ...
        <tbody>{datos.map((r,i) => {
          const val = r[campo] || 0;
          const prom = r.pj > 0 ? (val/r.pj).toFixed(1) : "0.0";
          return (
            <tr key={r.equipo.id}>
...
function RegisterPlayerModal({ onClose, showToast, onLogin }) {
  ...
  const handle = async () => {
    ...
    if (data.user || data.id) {
      ...
      if (token) {
        const jugRes = await fetch(`${SUPABASE_URL}/rest/v1/jugadores`, {
          method:"POST",
          headers:{ "apikey":SUPABASE_KEY, "Authorization":`Bearer ${token}`, "Content-Type":"application/json", "Prefer":"return=representation" },
          body:JSON.stringify({ user_id:userId, nombre_completo:form.nombre_completo, fecha_nacimiento:form.fecha_nacimiento||null, domicilio:form.domicilio, posicion_preferida:form.posicion_preferida })
        });
  ...
  if (success) return (
    <div className="ifutbol-overlay" onClick={onClose}>
      <div className="ifutbol-modal" onClick={e=>e.stopPropagation()} style={{ maxWidth:360, textAlign:"center" }}>
        ...
        <p style={{ fontSize:12, color:"var(--text-muted)", marginBottom:20 }}>Guarda este número, te identificará en todas las ligas</p>
...
function RegisterStaffModal({ onClose, showToast }) {
  ...
  const handle = async () => {
    ...
    setLoading(true); setError("");
    const data = await api("/auth/v1/signup", { method:"POST", body:JSON.stringify({ email:form.email, password:form.password }) });
    if (data.user || data.id) { setSuccess(true); }
    else { setError(data.msg || data.error_description || "Error al registrarse"); }
    setLoading(false);
  };
  ...
  if (success) return (
    <div className="ifutbol-overlay" onClick={onClose}>
      <div className="ifutbol-modal" onClick={e=>e.stopPropagation()} style={{ maxWidth:360, textAlign:"center" }}>
        ...
        <p style={{ color:"var(--text-sub)", marginBottom:24 }}>Tu solicitud de registro ha sido enviada al administrador. Te confirmarán tu acceso pronto.</p>
        <button className="btn btn-premium" style={{ width:"100%" }} onClick={onClose}>Ir al inicio →</button>
      </div>
    </div>
  );
